...
function App() {
  ...
  const animateCard = useCallback(async(value) => {
        let card = document.createElement('img');
        card.setAttribute("src", `/uno_assets_2d/PNGs/small/${value}.png`);
  ...
  const animateCardGet = useCallback(async() => {
    let card = document.createElement('img');
    card.setAttribute("src", `/uno_assets_2d/PNGs/small/card_back_alt.png`);
    card.setAttribute("id", `get` + turn);
    document.getElementById('container').appendChild(card);
    await delay(1000);
    card.remove();
}, [turn])
  ...
  const playAi = (async (deckAi, setter) => {
    ...
    if (temp === -1) {
      await animateCardGet();
      setter([...deckAi, getCards(1)])
      
    } else {
      await animateCard(getCardValue(deckAi[temp]));
